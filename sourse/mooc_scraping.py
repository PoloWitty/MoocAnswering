...
# COURSE = "模拟电子技术基础"
COURSES = ["电子线路设计、测试与实验（一）",'电子线路设计、测试与实验（二）',"模拟电子技术基础",'数字电子技术基础']
...
def run(playwright):
    for COURSE in COURSES:
        ...
        os.makedirs('./scrap_results/'+course_dir,exist_ok=True)
        ...
        for time in range(SCRAP_TIMES):
            ...
            for unit in units_list:
                ...
                try:
                    #新表就可以直接写
                    fp=open('./scrap_results/'+course_dir+'/'+unit_name+'.json','x',encoding='utf-8')
                    fp.write(json.dumps(res_problems, ensure_ascii=False))
                    fp.close()
                except:
                    #旧表就要做下数据的合并
                    old_table=pd.read_json('./scrap_results/'+course_dir+'/'+unit_name+'.json',encoding='utf-8')
                    ...
                    new_table.to_json('./scrap_results/'+course_dir+'/'+unit_name+'.json',force_ascii=False,orient='records')
